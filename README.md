...
A **web-based hospital management system** built using **PHP**, **MySQL**, and **Bootstrap** to streamline hospital workflows.  
This system enables **patients** to register and book appointments, **doctors** to view and manage their schedules, and **admins** to oversee hospital operations — all in one platform.
...
- Cancel appointments (updates visible to doctors).

### **Doctor Module**
- Login to access schedules.
- View and search appointments by patient contact.
- Cancel appointments when required.

### **Admin Module**
- Manage patients, doctors, and appointments.
- Add/remove doctors.
- View patient feedback and queries.
- Export details to Excel (planned feature).

---

## 🚀 Need to Work On (Future Enhancements)
1. Doctor acknowledgment for appointments.  
2. Email uniqueness check during registration.  
3. Password encryption & hide password in admin panel.  
4. Implement pagination for all list views.  
5. Fix duplicate records in billing when revisiting the same doctor.  
6. Add more fields in prescriptions.  
7. Add detailed payment records (date, amount, etc.).  
8. Implement export to Excel for admin reports.  
...
2. Install a text editor (**VS Code** or **Sublime Text**).  
3. Use any updated web browser (Google Chrome recommended).  
...
3. Move the project folder to `htdocs` inside the XAMPP directory.  
4. Open `localhost/phpmyadmin` and create a database named `myhmsdb`.  
5. Import the `myhmsdb.sql` file provided in the project.  
